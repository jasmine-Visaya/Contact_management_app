    ...
    <div class="detail-card">
      ...
      <div class="detail-header">
        ...
        <button
          ...
          @click="contact && $emit('toggle-favorite', contact)"
        >
      ...
      <div class="detail-body">
        ...
        <div class="detail-actions">
          ...
          <button class="detail-action-btn delete-btn-full" @click="contact && $emit('delete', contact.id)">
...
import { IonModal, IonBadge, IonIcon } from '@ionic/vue';
import {
...
/* Mobile: sheet from 60% up to 100%. Desktop ignores these. */
const breakpoints = computed(() => [0, 0.6, 1]);
const initialBreakpoint = 0.9;
...
.detail-card {
  ...
  overflow: hidden;
  /* Sheet gets rounded top corners from ion-modal host */
}

/* Header — gradient fills the safe area at top */
.detail-header {
  flex-shrink: 0;
  display: flex;
  align-items: center;
  gap: 8px;
  padding: 14px 16px;
  /* Add notch inset so gradient extends behind the status bar */
...
.detail-fav-btn { color: rgba(255, 255, 255, 0.7); }
.detail-fav-btn.favorited { color: #f6ad55; transform: scale(1.1); }

/* Body — bottom pads for home indicator */
.detail-body {
  flex: 1 1 auto;
  overflow-y: auto;
...
.detail-avatar-wrapper { position: relative; display: inline-block; }
...
.avatar-edit-btn:hover:not(:disabled) { transform: scale(1.1); }

:global(html.dark-theme) .avatar-edit-btn { border-color: #0f1117; }
...
.detail-action-btn ion-icon { font-size: 1.15rem; }
...
.delete-btn-full:hover { background: var(--delete-bg-hover); }
...
@media (min-width: 768px) {
  .detail-avatar { width: 110px; height: 110px; font-size: 2.5rem; }
...
<style>
/* Mobile: bottom sheet with rounded top, sits above home indicator */
ion-modal.detail-modal {
  --border-radius: 24px 24px 0 0;
  --background: var(--app-bg);
  --backdrop-opacity: 0.5;
  /* keep the sheet from touching the very top status bar */
...
html.dark-theme ion-modal.detail-modal {
  --background: #0f1117;
}
